...
from cmeplus.releases.models import Release, UpdateStatus
...
def test_semver_parsing_and_comparisons():
    # Regular SemVer
    assert parse_semver("0.1.0") == (0, 1, 0, 1, "")
    assert parse_semver("v0.10.0") == (0, 10, 0, 1, "")
    assert parse_semver("0.2.0") == (0, 2, 0, 1, "")
...
def test_same_version_update_prevention():
    ...
    mock_service = MagicMock(spec=ReleaseService)
    mock_service.check_update.return_value = UpdateStatus(
        installed="0.1.0",
        latest="0.1.0",
    ...
    engine = UpdateEngine(console=console_out, release_service=mock_service)

    with patch("cmeplus.update.engine.get_installed_version", return_value="0.1.0"):
...
def test_update_newer_version_prompts_confirmation():
    ...
    # User cancels prompt
    with (
        patch("cmeplus.update.engine.get_installed_version", return_value="0.1.0"),
...
def test_update_to_specific_release_downgrade_confirmation():
    ...
    mock_service.get_release_by_version.return_value = (mock_release, None)
    mock_service.is_downgrade.return_value = True
    ...
    with (
        ...
        assert ret == 0
        output = rich_console.export_text()
        assert "You are about to downgrade CrackMapExec+" in output
...
def test_update_to_invalid_release():
    ...
    console_out = OutputConsole(console=rich_console)
    mock_service = MagicMock(spec=ReleaseService)
    mock_service.get_release_by_version.return_value = (None, "Not found on official GitHub repository.")
    engine = UpdateEngine(console=console_out, release_service=mock_service)
...
def test_update_engine_check_mode_non_destructive():
    ...
    mock_report.total = 25
    mock_report.passed_count = 25
    mock_report.warn_count = 0
    mock_report.failed_count = 0
    mock_report.get_by_category.return_value = []
